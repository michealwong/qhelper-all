...
extern "C" {
    ...
    /** XML文件输入输出结构体对象的定义
    */
    typedef struct tagTDRInOutputSrc
    {
     ...
     * 集成了 tdr_output/tdr_output_fp/tdr_output_file 接口的功能，并且支持输出转义字符
     *
     * @note 调用前需要调用tdr_data_inoutput_init进行输入参数的初始化，或者将输入参数memset为0
     *
     * @note 仅仅支持八进制格式的转义字符，例如\\012 表示 回车符\\n
     *
     * @note stInOutputSrc成员输出方式优先级：szFileName > fp > stXmlBuf。szFileName!=NULL时，以szFileName为准，
     * 忽略fp值和stXmlBuf，以此类推
     *
     * @note 如果处理失败，获取错误信息的方法:
     *	- 根据返回值，调用tdr_error_string()可以获取出错信息
     ...
     * 集成了 tdr_input/tdr_input_fp/tdr_input_file 接口的功能，并且支持输入转义字符
     *
     * @param[in] a_pstIOWrap 功能控制信息
     *
     * @note 调用前需要调用tdr_data_inoutput_init进行输入参数的初始化，或者将输入参数memset为0
